...
class Gamepad:
    ...
    def move_joystick(self, index=None, x=None, y=None):
        ...
        if index is not None:
            if x is not None:
                self._joysticks[index][0] = self._validate_joystick_value(x)
            if y is not None:
                self._joysticks[index][1] = self._validate_joystick_value(y)

            self._send()

    def move_analog(self, index=None, value=None):
        """Set and send the given joystick values.
        ...
        if index is not None and value is not None:
            self._analogs[index] = self._validate_analog_value(value)
    ...
    @staticmethod
    def _validate_analog_value(value):
        if not -127 <= value <= 127:
            raise ValueError("Analog value must be in range 0 to 127")
        return value
